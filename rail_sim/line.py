...
class Line:
	def __init__(
		...
		# Store original station list (may be strings or ints), first assert that no same stations appear twice in a row (can loopback to the same station, but needs to go somewhere first)
		assert not any(station_list[i] == station_list[i-1] for i in range(1, len(station_list))), "There are stations that appear twice in a row."
